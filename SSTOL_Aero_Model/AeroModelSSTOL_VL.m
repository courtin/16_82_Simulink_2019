	function [CX,CL,CY,Cl,Cm,Cn, vis_data]	=	AeroModelSSTOL_VL(x,u,Mach,alphar,betar,V,airplane)
%	SSTOL Aero Model
...
%   ====================================
    ...
    motors = [dCJ_TL,dCJ_BL,dCJ_BL,dCJ_BL,dCJ_BR,dCJ_BR,dCJ_BR,dCJ_TR];
...
%	====================================
    ...
    %C_L_wing
    %This uses the linearized WVL method to compute logitudinal forces
    %only, currently. 
    
    flaps = [u(4), u(4), u(5), u(5)];

    [CLw,CXw, CMw, eps] = solve_VL(alpha_w, motors, flaps, airplane, 0);
    eps = -mean(eps);   %Fliping sign for consistenceny w/ downwash defined positive down
...
%     end     
    cla_t = 2*pi;
    CLah = (cla_t*AR)/(2+sqrt(4*(AR*2*pi/cla_t)^2*(1+tan(sweep_h)^2)));
...
%     cx_right = getCXwing(a_w_deg,dCJ_BR,CT_BR,flap_R_deg,airplane);
    ...
    CDp = .05;  %Placeholder, this has little effect on the high-lift cases
