function [CL,CL2,CX] = run_NWVL2(alpha_in, dCJ_in, dF_in, verbose)

useTAT = 0;
blow_center = 1;
%Set up geometry
%b = 13;
[vortex, cs, g_dCJ1, g_flap, N, b, Sref, vortex_cs, ra, rb] = initialize_geometry(verbose);
y_fuse = 1.5;
...
y = vortex(2,:);
Gam0 = .5*vortex_cs(N)*cl_nom_b*Vinf;
Gam = Gam0.*(1-(2.*y./b).^2).^(.5);
Gami = Gam;


%Solve gamma using matlab build-in solver
f = @(Gam) get_Gam_R(Gam, vortex, ra, rb, alpha,dCJs,dF,N,vortex_cs,unblown, useTAT); 

Gam = fsolve(f, Gam, optimoptions('fsolve','FunctionTolerance', .01));
%Gam = fminsearch(f, Gam)
[R, Gam_new] = get_Gam_R(Gam, vortex, ra, rb, alpha,dCJs,dF,N,vortex_cs,unblown, useTAT); 
Gamb = Gam;
%R
...
CL2 = trapz(vortex(2,:),cls.*vortex_cs')/Sref;
CXp  = trapz(vortex(2,:),cxs.*vortex_cs')/Sref;
CX = CXp;
AR = b^2/Sref;
CDi2 = CL^2/(pi*AR);
if verbose
